...
def investment_module():
    ...
    win = tk.Toplevel()
    win.title("Investment Module")
    win.geometry("400x600")
    win.configure(bg="#1e1f6e")

    # Título
    tk.Label(win, text="Investment Opportunities", font=("Helvetica", 14, "bold"), bg="#1e1f6e", fg="white").pack(pady=10)

    # Entrada para monto a invertir
    tk.Label(win, text="How much would you like to invest?", bg="#1e1f6e", fg="white").pack()
    ...
    def display_prices():
        ...
        for symbol, data in prices.items():
            ...
            tk.Label(frame, text=f"Current Price: ${data['current']:.2f}", bg="#1e1f6e", fg="white").pack(anchor="w")
            tk.Label(frame, text=f"1 Hour Ago: ${data['one_hour_ago']:.2f}", bg="#1e1f6e", fg="white").pack(anchor="w")
            tk.Label(frame, text=f"1 Day Ago: ${data['one_day_ago']:.2f}", bg="#1e1f6e", fg="white").pack(anchor="w")
            tk.Label(frame, text="-"*30, bg="#1e1f6e", fg="white").pack()

    # Botón para mostrar los precios
    tk.Button(win, text="Show Stock Prices", command=display_prices, bg="#635bff", fg="white").pack(pady=10)

    # Botón para volver
    tk.Button(win, text="Back", command=win.destroy, bg="#48aaff", fg="white").pack(pady=10)
